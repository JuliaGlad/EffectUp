...
class AddYearlyGoalDialogFragment(
    val isNew: Boolean,
    val yearId: String,
    val year: String
) : DialogFragment() {
    private val viewModel: AddYearlyGoalViewModel by viewModels()
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding.buttonAdd.setOnClickListener {
            ...
            if (binding.editLayoutTitle.getText()?.isNotEmpty() == true) {
                ...
                goalId?.let { goalId -> goal?.let { goal -> viewModel.addGoal(isNew, yearId, year, goalId, goal)}}
            }
        }
